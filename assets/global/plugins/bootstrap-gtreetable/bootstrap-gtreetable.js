...
(function ($) {
    ...
    GTreeTableNode.prototype = {
        ...
        insert: function (position, oRelatedNode) {
            ...
            if (position === 'before') {
                ...
                if (oRelatedNode.isExpanded()) {
                    oLastChild = oRelatedNode.getDescendants({
                        depth: 1,
                        index: -1,
                        includeNotSaved: true
                    });
        ...
        _move: function (oDestination, position) {
            var oNode = this,
                oNodeDescendants = oNode.getDescendants({
                    depth: -1,
                    includeNotSaved: true
                }),
            ...
            if (this.manager.options.cache > 0) {
                this.manager.cacheManager.synchronize('move', oNode, {
                    'oOldNode': oOldNode,
                    'oldIP': oldIP
                });
            }
        ...
        sort: function () {
            ...
            if (oSiblings.length > 0) {
                var oDescendants = !oNode.isExpanded() ? [] : oNode.getDescendants({
                        depth: -1,
                        includeNotSaved: true
                    }),
                    oRelated;
