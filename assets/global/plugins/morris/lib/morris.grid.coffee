...
# find the container to draw the graph in
    ...
    @el.bind 'mousemove', (evt) =>
      ...
      if @selectFrom
        left = @data[@hitTest(Math.min(x,
          @selectFrom))]._x
        right = @data[@hitTest(Math.max(x,
          @selectFrom))]._x
        width = right - left
        @selectionRect.attr({
          x: left,
    ...
    if @options.rangeSelect
      @selectionRect = @raphael.rect(0,
        0,
        0,
        @el.innerHeight())
      .attr({
          fill: @options.rangeSelectColor,
          stroke: false
        })
      .toBack()
      .hide()
    ...
    @el.css('-webkit-tap-highlight-color',
      'rgba(0,0,0,0)')
...
#
  setData: (data, redraw = true) ->
    ...
    if @options.goals.length > 0
      ...
      ymin = if ymin? then Math.min(ymin,
        minGoal) else minGoal
      ymax = if ymax? then Math.max(ymax,
        maxGoal) else maxGoal
    ...
    if typeof boundaryOption is 'string'
      if boundaryOption[0..3] is 'auto'
        if boundaryOption.length > 5
          suggestedValue = parseInt(boundaryOption[5..],
            10)
          return suggestedValue unless currentValue?
          Math[boundaryType](currentValue,
            suggestedValue)
        else
    ...
    if @elementWidth != w or @elementHeight != h or @dirty
      ...
      if @options.axes in [true,
        ...
        bottomOffsets = for i in [0...@data.length]
          @measureText(@data[i].text,
            -@options.xLabelAngle).height
...
#
  measureText: (text, angle = 0) ->
    tt = @raphael.text(100,
      ...
      text)
    .attr('font-size',
      @options.gridTextSize)
    .attr('font-family',
      @options.gridTextFamily)
    .attr('font-weight',
      @options.gridTextWeight)
    .rotate(angle)
    ret = tt.getBBox()
    tt.remove()
    ret
...
#
  drawGrid: ->
    return if @options.grid is false and @options.axes not in [true,
                                                               'both',
                                                               'y']
    for lineY in @grid
      y = @transY(lineY)
      if @options.axes in [true,
                           'both',
                           'y']
        @drawYAxisLabel(@left - @options.padding / 2,
          y,
          @yAxisFormat(lineY))
      if @options.grid
...
#
  ...
  endRange: (x) ->
    if @selectFrom
      start = Math.min(@selectFrom,
        x)
      end = Math.max(@selectFrom,
        x)
