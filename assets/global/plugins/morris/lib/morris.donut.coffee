...
# @private
  setLabels: (label1, label2) ->
    ...
    text1scale = Math.min(maxWidth / text1bbox.width,
...
class Morris.DonutSegment extends Morris.EventEmitter
  constructor: (@cx, @cy, @inner, @outer, p0, p1, @color, @backgroundColor, @index, @raphael) ->
    ...
    @path = @calcSegment(@inner + 3,
      @inner + @outer - 5)
    @selectedPath = @calcSegment(@inner + 3,
      @inner + @outer)
