...
# @private
  hoverContentForRow: (index) ->
    ...
    if typeof @options.hoverCallback is 'function'
      content = @options.hoverCallback(index,
        @options,
        content,
        row.src)
    [content,
...
# @private
  generatePaths: ->
    @paths = for i in [0...@options.ykeys.length]
      ...
      x: r._x,
      y: r._y[i]
      } for r in @data when r._y[i] isnt undefined)
...
#
  draw: ->
    @drawXAxis() if @options.axes in [true,
                                      'both',
                                      'x']
    @drawSeries()
...
# draw x axis labels
    ...
    drawLabel = (labelText, xpos) =>
      label = @drawXAxisLabel(@transX(xpos),
        ypos,
        labelText)
...
# column label
        ...
        labels = Morris.labelSeries(@xmin,
          @xmax,
          @width,
          @options.xLabels,
          @options.xLabelFormat)
    else
      labels = ([row.label,
...
# @private
  ...
  _drawLineFor: (index) ->
    path = @paths[index]
    if path isnt null
      @drawLinePath path, @colorFor(null,
        index,
        'line'), index
...
# @private
  ...
  drawLinePath: (path, lineColor, lineIndex) ->
    ...
    .attr('stroke',
      lineColor)
    .attr('stroke-width',
      @lineWidthForSeries(lineIndex))

  drawLinePoint: (xPos, yPos, pointColor, lineIndex) ->
    @raphael.circle(xPos,
      yPos,
      @pointSizeForSeries(lineIndex))
    .attr('fill',
      pointColor)
    .attr('stroke-width',
      @pointStrokeWidthForSeries(lineIndex))
    .attr('stroke',
      @pointStrokeColorForSeries(lineIndex))
...
# @private
  pointGrowSeries: (index) ->
    Raphael.animation
      r: @pointSizeForSeries(index) + 3, 25, 'linear'

# @private
  pointShrinkSeries: (index) ->
    Raphael.animation
      r: @pointSizeForSeries(index), 25, 'linear'
...
minutesSpecHelper = (interval) ->
  ...
  start: (d) -> new Date(d.getFullYear(),
    d.getMonth(),
    d.getDate(),
    d.getHours())
...
Morris.LABEL_SPECS =
  "decade":
    ...
    start: (d) -> new Date(d.getFullYear() - d.getFullYear() % 10,
      0,
      1)
    fmt: (d) -> "#{d.getFullYear()}"
