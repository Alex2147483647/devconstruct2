describe 'Morris.Grid#setData', ->
  ...
  describe 'sorting', ->
    it 'should sort data when parseTime is true', ->
      line = Morris.Line
        ...
        ykeys: ['y']
        labels: ['y']
      line.data.map((row) -> row.label).should.deep.equal ['2012 Q1',
                                                           '2012 Q2',
                                                           '2012 Q3',
                                                           '2012 Q4']
    ...
    it 'should not sort data when parseTime is false', ->
      line = Morris.Line
        ...
        labels: ['y']
        parseTime: false
      line.data.map((row) -> row.label).should.deep.equal [1,
                                                           4,
                                                           3,
                                                           2]
  ...
  describe 'timestamp data', ->
    ...
    it 'should use a user-supplied formatter for labels', ->
      ...
      line.data.map((row) -> row.label).should.deep.equal ['2012-1-1',
                                                           '2012-1-2',
                                                           '2012-1-3',
                                                           '2012-1-4']
  ...
  it 'should parse y-values in strings', ->
    ...
    line.ymin.should == 12
    line.ymax.should == 16
    line.data.map((row) -> row.y).should.deep.equal [[13.5],
                                                     [12],
                                                     [16],
                                                     [14]]
