describe 'Morris.Line', ->
  ...
  it 'should make point styles customizable', ->
    ...
    chart.colorFor(chart.data[0],
      0,
      'point').should.equal chart.colorFor(chart.data[0],
      0,
      'line')
    chart.colorFor(chart.data[1],
      1,
      'point').should.equal red
  ...
  describe 'generating column labels', ->
    it 'should use user-supplied x value strings by default', ->
      chart = Morris.Line
        ...
        ykeys: ['y']
        labels: ['dontcare']
      chart.data.map((x) -> x.label).should == ['2012 Q1',
                                                '2012 Q2']
    ...
    it 'should use a default format for timestamp x-values', ->
      ...
      chart = Morris.Line
        ...
        ykeys: ['y']
        labels: ['dontcare']
      chart.data.map((x) -> x.label).should == [d2.toString(),
                                                d1.toString()]
    ...
    it 'should use user-defined formatters', ->
      ...
      chart.data.map((x) -> x.label).should == ['2012/1/1',
                                                '2012/1/2']
...
# the chart.
  ...
  describe '#createPath', ->
    it 'should generate a smooth line', ->
      ...
      path = Morris.Line.createPath(testData,
        true,
        20)
