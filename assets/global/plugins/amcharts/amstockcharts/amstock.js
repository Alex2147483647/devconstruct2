AmCharts.AmStockChart = AmCharts.Class({
    ...
    addPeriodSelector: function () {
        ...
        if (a) {
            ...
            var k = this.leftContainer, f = this.rightContainer, h = this.centerContainer, g = this.panelsContainer, n = a.width + 2 * c + "px";
            switch (a.position) {
                case "left":
    ...
    addDataSetSelector: function () {
        ...
        if (a) {
            ...
            var b = a.position, c = this.panelsSettings.panelSpacing,
                d = document.createElement("div");
            this.dssContainer = d;
            var e = this.leftContainer, k = this.rightContainer, f = this.centerContainer, h = this.panelsContainer, c = a.width + 2 * c + "px";
            switch (b) {
                case "left":
    ...
    zoomChart: function (a, b) {
        ...
        if (e && k) {
            ...
            d.currentPeriod = f;
            var l = AmCharts.extractPeriod(f), y = AmCharts.getPeriodDuration(l.period, l.count);
            ...
            d.endDate = b;
            m && m.zoom(a, b);
            if (a.getTime() != d.previousStartDate.getTime() || b.getTime() != d.previousEndDate.getTime())m = {type: "zoomed"}, m.startDate = a, m.endDate = b, m.chart = d, m.period = f, d.fire(m.type, m), d.previousStartDate = new Date(a), d.previousEndDate = new Date(b)
        }
        ...
        if (0 < this.dataSets.length) {
            ...
            for (b = 0; b < a.length; b++) {
                ...
                for (f = 0; f < k.length; f++) {
                    var h = k[f], h = AmCharts.processObject(h, AmCharts.StockGraph, this.theme);
                    k[f] = h;
    ...
    choosePeriod: function (a, b, c) {
        var d = this.categoryAxesSettings, e = d.groupToPeriods, k = e[a], e = e[a + 1], f = AmCharts.extractPeriod(k), f = AmCharts.getPeriodDuration(f.period, f.count), h = b.getTime(), g = c.getTime(), d = d.maxSeries;
        return (g - h) / f > d && 0 < d && e ? this.choosePeriod(a + 1, b, c) : k
    },
    handleCursorChange: function (a) {
        var b =
            a.target, c = a.position, d = a.zooming;
        a = a.index;
...
AmCharts.PeriodSelector = AmCharts.Class({
    ...
    write: function (a) {
        ...
        if (b.inputFieldsEnabled) {
            ...
            b.startDateField =
                g;
            if (d)h = b.width - 6 + "px", AmCharts.addBr(f); else {
                var h = b.inputFieldWidth + "px", n = document.createTextNode(" ");
                f.appendChild(n)
            }
            g.style.width = h;
    ...
    handleCalChange: function (a) {
        var b = this.dateFormat, c = AmCharts.stringToDate(this.startDateField.value, b), b = this.chart.getLastDate(AmCharts.stringToDate(this.endDateField.value, b));
        try {
...
AmCharts.StockPanel = AmCharts.Class({
    ...
    createDrawIcons: function () {
        var a = this, b = a.iconSize, c = a.container, d = a.pathToImages, e = a.realWidth - 2 * b - 1 - a.marginRight, k = AmCharts.rect(c, b, b, "#000", .005), f = AmCharts.rect(c, b, b, "#000", .005);
        f.translate(b + 1, 0);
...
AmCharts.parseStockData = function (a, b, c, d, e) {
    ...
    if (h) {
        ...
        for (m = 0; m < t; m++) {
            var z = f[m], x = z[h], x = x instanceof Date ? AmCharts.newDate(x, b) : e ? AmCharts.stringToDate(x, e) : new Date(x), B = x.getTime(), A = {};
            for (r = 0; r < y; r++)A[l[r].toField] = z[l[r].fromField];
            var q;
            for (q = g; q < n; q++) {
                r = c[q];
                var s = AmCharts.extractPeriod(r), C = s.period,
                    E = s.count, u, p;
                if (q == g || B >= v[r] || !v[r]) {
                    w[r] = {};
                    ...
                    if (q == g)for (var F in z)z.hasOwnProperty(F) &&
                    (w[r][F] = z[F]);
                    w[r][h] = AmCharts.newDate(x, b)
                } else for (C = 0; C < y; C++)s = l[C].toField, u = w[r], m == t - 1 && (u[h] = AmCharts.newDate(x, b)), p = Number(A[s]), isNaN(p) || (isNaN(u[s + "Low"]) && (u[s + "Low"] = p), p < u[s + "Low"] && (u[s + "Low"] = p), isNaN(u[s + "High"]) && (u[s + "High"] = p), p > u[s + "High"] && (u[s + "High"] = p), u[s + "Close"] = p, E = AmCharts.getDecimals(u[s + "Sum"]), D = AmCharts.getDecimals(p), u[s + "Sum"] += p, u[s + "Sum"] = AmCharts.roundTo(u[s + "Sum"], Math.max(E, D)), u[s + "Count"]++, u[s + "Average"] = u[s + "Sum"] / u[s + "Count"])
            }
        }
    }
...
AmCharts.parseEvents = function (a, b, c, d, e, k) {
    ...
    for (n = 0; n < g; n++) {
        ...
        for (var x = 0; x < f.length; x++)if (l = f[x], z = l.graph, AmCharts.isString(z) && (z = AmCharts.getObjById(v, z)))l.graph = z
    }
    for (var B in h)if (h.hasOwnProperty(B)) {
        ...
        for (s =
            ...
            for (n = 0; n < g; n++)for (w = b[n], v = w.graphs, t = v.length, m = 0; m < t; m++) {
                r = v[m];
                var p = {};
                p.eventDispatcher = e;
                p.eventObjects = [];
...
AmCharts.DataSetSelector = AmCharts.Class({
    ...
    write: function (a) {
        ...
        if (0 < b.countDataSets("showInCompare") &&
            1 < e.length)if (k ? (AmCharts.addBr(a), AmCharts.addBr(a)) : (c = document.createTextNode(" "), a.appendChild(c)), c = document.createTextNode(AmCharts.lang.compareText || b.compareText), a.appendChild(c), g = b.listCheckBoxSize, k) {
            AmCharts.addBr(a);
            f = document.createElement("div");
            a.appendChild(f);
...
AmCharts.StackedBullet = AmCharts.Class({
    ...
    drawPin: function (a) {
        ...
        return this.drawRealPolygon(a, [0,
            c / 2, c / 2, -c / 2, -c / 2, 0], [0, b * c / 4, b * (c + c / 4), b * (c + c / 4), b * c / 4, 0])
    },
    drawSign: function (a) {
        var b = -1;
        this.stackDown && (b = 1);
